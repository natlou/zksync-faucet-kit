...
async function mintTestTokens(token, amount) {
    ...
    console.log(`Mint ${amount} test token ${token}..`);
    
    const handle = await faucetContract.mint(token, amount, signer.address);
...
async function execute() {
    ...
    await mintTestTokens(args[0], args[1]);
}
